...
export const useAst = () => {
  const astToModules = useCallback((ast: ScriptNode): EnhancedModuleType[] => {
    ...
    const processCommand = (
      ...
      } else if (command.type === 'Command' && command.name && command.name.text) {
  ...
  const modulesToAst = useCallback((modules: EnhancedModuleType[]): ScriptNode => {
    const buildAst = (
      ...
      let currentAst: ASTType = (Plugins.get(modules[0].type) || genericPlugin).compile(modules[0]);
  ...
  const compileCommand = useCallback((modules: EnhancedModuleType[]): string => {
